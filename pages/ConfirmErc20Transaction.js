...
import { sendEvmToken , Evm_estimatedGas_Evm ,  getSolBalance, getEVMBalance } from '../utils/function';
...
const ConfirmErc20Transaction = ({route, navigation}) => {
  ...
  const confirmAndSend = async () => {
    try {
      setLoader(true)
      let response = await sendEvmToken(trxDetail?.privateKey, trxDetail?.recipientAddress, trxDetail?.amountWei, trxDetail?.tokenAddress, trxDetail?.chain);
...
export default ConfirmErc20Transaction;

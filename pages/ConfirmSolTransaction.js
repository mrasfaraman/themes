...
import { sendEvmNative , Sol_estimatedGas, sendSolNative } from '../utils/function';
import MaroonSpinner from '../components/Loader/MaroonSpinner';
import { useAuth } from '../context/AuthContext';
import { ALERT_TYPE, Dialog, AlertNotificationRoot, Toast } from 'react-native-alert-notification';

const ConfirmSolTransaction = ({route, navigation}) => {
  ...
  const confirmAndSend = async () => {
    try {
      setLoader(true)
      let response = await sendSolNative(trxDetail?.privateKey, trxDetail?.recipientAddress, trxDetail?.amount);
  ...
  const getEstimatedGas = async (data) => {
    try{
      setLoader(true)
      console.log(data?.privateKey, data?.recipientAddress, data?.amount)
      let gasData = await Sol_estimatedGas(data?.privateKey, data?.recipientAddress, data?.amount)
...
export default ConfirmSolTransaction;
